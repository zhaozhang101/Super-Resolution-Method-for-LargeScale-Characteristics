...
class tesloss_rmse(nn.Module):
    ...
    def forward(self, yhat, gtruth, mask):
        ...
        gtruth = torch.mul(gtruth, mask)
        predot = torch.sum(mask, dim=[2, 3])
        ans = torch.sum(self.loss2(yhat, gtruth), dim=[2, 3]) / predot
        # print("ans :", ans)
        ans = torch.sqrt(ans)
        ans = torch.mean(ans, dim=0)
        # print("origin : f{}", ans)
...
class tesloss_ame(nn.Module):
    ...
    def forward(self, yhat, gtruth, mask):
        ...
        predot = torch.sum(mask, dim=[2, 3])
        sub = yhat - gtruth
        ans = torch.sum(sub, dim=[2, 3]) / predot
        ans = torch.mean(ans, dim=0)
        ans = torch.abs(ans)
        return ans  # c
...
class bploss(nn.Module):
    def __init__(self, length, args):
        ...
        # self.coefficient = torch.cat((self.coefficient1,self.coefficient2,self.coefficient3,self.coefficient4,self.coefficient5),dim=0).to(args.device)
        # self.coefficient  = nn.Parameter(torch.randn(size=(self.length,1)))

    def forward(self, loss, std):
        self.coefficient = torch.cat(
            (self.coefficient1, self.coefficient2,self.coefficient3,self.coefficient4,self.coefficient5,self.coefficient6), dim=0)
